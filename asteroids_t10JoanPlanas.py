...
class World(object):
    ...
    def handle_keydown(self, evento):
    ...
    def handle_keyup(self, evento):
         if evento.type == pygame.KEYUP:
            ...
            if evento.key == pygame.K_SPACE:
                ...
                self.sprites.add(Bullet(origin.to_position(), direction, magnitude))
...
def collisionNau():
    RED = (255, 0, 0)
    for spriteN in world.sprites:
         if isinstance(spriteN, Player):
            for spriteA in world.sprites:
                if isinstance(spriteA, Asteroid):
                    if (Vector.from_position(spriteN.rect.center)-Vector.from_position(spriteA.rect.center)).magnitude() <= 20:
                        print("entra nau")
                        world.running = False
...
def collisionBala():
    for spriteB in world.sprites:
         if isinstance(spriteB, Bullet):
            for spriteA in world.sprites:
                if isinstance(spriteA, Asteroid):
                    if (Vector.from_position(spriteB.rect.center)-Vector.from_position(spriteA.rect.center)).magnitude() <= 20:
                        print("entraTir")
                        spriteA.duration = 0
                        spriteB.duration = 0
...
def main():
    ...
    colisionUpdate = Thread(target=update_collision)
    supdate.start()
    colisionUpdate.start()
    while world.running:
